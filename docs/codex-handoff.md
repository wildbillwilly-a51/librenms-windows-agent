...
- Current state: Agent 0.6.16 and overlay 0.6.20 are built and published on
  GitHub `main`. The public MSI, versioned config, overlay archive, manifest,
  and installers were downloaded back and verified. The local agent classifies
  expected services and only the active Horizon certificate. The central
  collector publishes independent health scopes, explicit redundancy/component
  reasons, bounded condition history, fail-soft vendor metrics, and grouped
  informational observations. The UI consumes those classifications.
...
- Validation completed: portable .NET tests, shared C#/PHP policy fixtures,
  ...
  public-safety scanning. GitHub `main` and commit-addressed release downloads
  matched the committed SHA-256 manifest. Live Horizon deployment was
